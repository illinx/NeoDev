...
int Init_2xSaI (Uint32 BitFormat)
{
	...
	delta=SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 226, 16, 0xF800, 0x07E0, 0x001F, 0x0000);
...
/*(Uint8 *srcPtr, Uint32 srcPitch,
    ...
    Uint8 *srcPtr=(Uint8 *)(video_buffer->pixels)+16;
	Uint8 *deltaPtr=(Uint8 *)(delta->pixels);
	Uint32 srcPitch=320*2;
	Uint8 *dstPtr=(Uint8 *)(screen->pixels)+(1280*14)+32;
	Uint32 dstPitch=640*2;
	int width=304;
	int height=223;
	Uint16 *bP;
    Uint8  *dP;
	...
	if(SDL_MUSTLOCK(delta)) SDL_LockSurface(delta);
...
//--------------------------------------
	...
	if(SDL_MUSTLOCK(delta)) SDL_UnlockSurface(delta);
	SDL_Flip(screen);
}

void SuperEagle(void) 
{

/*(Uint8 *srcPtr, Uint32 srcPitch, Uint8 *deltaPtr, 
